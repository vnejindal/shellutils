...
## Postgress SQL server installer script for Linux 
...
LOGFILE=/tmp/pg_install.log
TMP_PGSQL_CONFIG=/tmp/postgresql.conf.tmp
TMPFILE=/tmp/pgsql.tmp
...
echo "=================" >> $LOGFILE
logit "Installing postgress sql:"
#vne::tbd:: log the version available and being installed 
date >> $LOGFILE


SHARED_BUFFERS=1GB
EFFECTIVE_CACHE_SIZE=1GB
MAINT_WORK_MEMORY=512M
WORK_MEMORY=10485kB
#CHECKPOINT_COMPLETION_TARGET=0.9

SYSTEM_MEMORY=`free -g | grep Mem | awk '{print $2}'`
NUM_CPUS=`cat /proc/cpuinfo | grep processor | wc -l`
logit "System Memory: $SYSTEM_MEMORY"
create_pg_config()
{
        #set pg config variables 
	echo "system memory is $SYSTEM_MEMORY GB"
	if [ $SYSTEM_MEMORY -le 4 ]; then
		SHARED_BUFFERS=1GB
		EFFECTIVE_CACHE_SIZE=1GB
		MAINT_WORK_MEMORY=512MB
		WORK_MEMORY=10485kB
	elif [ $SYSTEM_MEMORY -le 8 ]; then 
		SHARED_BUFFERS=2GB
		EFFECTIVE_CACHE_SIZE=6GB
		MAINT_WORK_MEMORY=512MB
		WORK_MEMORY=10485kB
	elif [ $SYSTEM_MEMORY -le 16 ]; then 
		SHARED_BUFFERS=4GB
		EFFECTIVE_CACHE_SIZE=12GB
		MAINT_WORK_MEMORY=1GB
		WORK_MEMORY=20971kB
	elif [ $SYSTEM_MEMORY -le 32 ]; then 
		SHARED_BUFFERS=8GB
		EFFECTIVE_CACHE_SIZE=24GB
		MAINT_WORK_MEMORY=2GB
		WORK_MEMORY=41943kB
	elif [ $SYSTEM_MEMORY -le 64 ]; then 
		SHARED_BUFFERS=16GB
...
update_pg_config()
{
	...
	SKIP=`awk '/^__PG_CONFIG_FOLLOWS__/ { print NR + 1; exit 0; }' $0`
	TAIL="tail -n+$SKIP"
	$TAIL $0 > $TMP_PGSQL_CONFIG

	logit "updating pg config"
	REGEX="s|@CONFIG_SHARED_BUFF@|$SHARED_BUFFERS|"
	sed "$REGEX" $TMP_PGSQL_CONFIG > $TMPFILE
	mv $TMPFILE $TMP_PGSQL_CONFIG > /dev/null 2>&1

	REGEX="s|@CONFIG_EFFECTIVE_CACHE_SIZE@|$EFFECTIVE_CACHE_SIZE|"
	sed "$REGEX" $TMP_PGSQL_CONFIG > $TMPFILE
	mv $TMPFILE $TMP_PGSQL_CONFIG > /dev/null 2>&1

	REGEX="s|@CONFIG_MAINT_WORK_MEMORY@|$MAINT_WORK_MEMORY|"
	sed "$REGEX" $TMP_PGSQL_CONFIG > $TMPFILE
	mv $TMPFILE $TMP_PGSQL_CONFIG > /dev/null 2>&1

	REGEX="s|@CONFIG_WORK_MEMORY@|$WORK_MEMORY|"
	sed "$REGEX" $TMP_PGSQL_CONFIG > $TMPFILE
	mv $TMPFILE $TMP_PGSQL_CONFIG > /dev/null 2>&1

	REGEX="s|@CONFIG_IP_ADDR@|$PGSQL_IP_ADDRESS|"
	sed "$REGEX" $TMP_PGSQL_CONFIG > $TMPFILE
	mv $TMPFILE $TMP_PGSQL_CONFIG > /dev/null 2>&1

	REGEX="s|@CONFIG_PORT@|$PGSQL_PORT|"
	sed "$REGEX" $TMP_PGSQL_CONFIG > $TMPFILE
	mv $TMPFILE $TMP_PGSQL_CONFIG > /dev/null 2>&1
...
update_pg_hba()
{
   HBA_FILE=$PG_CONF_PATH/pg_hba.conf
   cp $HBA_FILE $HBA_FILE.orig
   ACCESS_LINE="host    all             all             $PGSQL_HBA_ACCESS_IP/$PGSQL_HBA_ACCESS_SUBNET       trust"
...
echo "postgress installed successfully"

exit 0

__PG_CONFIG_FOLLOWS__
# -----------------------------
...
listen_addresses = '@CONFIG_IP_ADDR@'		# what IP address(es) to listen on;
					# comma-separated list of addresses;
					# defaults to 'localhost'; use '*' for all
					# (change requires restart)
port = @CONFIG_PORT@                    # (change requires restart)
...
shared_buffers = @CONFIG_SHARED_BUFF@   # min 128kB
					# (change requires restart)
...
work_mem = @CONFIG_WORK_MEMORY@				# min 64kB
#hash_mem_multiplier = 1.0		# 1-1000.0 multiplier on hash table work_mem
maintenance_work_mem = @CONFIG_MAINT_WORK_MEMORY@		# min 1MB
...
effective_cache_size = @CONFIG_EFFECTIVE_CACHE_SIZE@
